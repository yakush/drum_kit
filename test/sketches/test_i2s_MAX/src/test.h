...
i2s_config_t test_i2s_config = {
    .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX),
    .sample_rate = 44100, //.sample_rate = 36000,
...
i2s_pin_config_t test_pin_config = {
...
int i2s_write_sample_2_bytes(uint32_t sample)
{
    return i2s_write_bytes((i2s_port_t)i2s_num, (const char *)&sample, sizeof(uint16_t), 100);
}
...
int read2bytes(File file, uint32_t *chunkId)
{
    int n = file.read((uint8_t *)chunkId, sizeof(uint16_t));
    return n;
}
    ...
    if (root)
    {
        ...
        while (root.available())
        {
            ...
            case HEADER_DATA:
                ...
                i2s_driver_install((i2s_port_t)i2s_num, &test_i2s_config, 0, NULL);
                i2s_set_pin((i2s_port_t)i2s_num, &test_pin_config);
            ...
            case DATA:
                ...
                uint32_t data;
                n = read2bytes(root, &data);
                i2s_write_sample_2_bytes((uint16_t)data);
                i2s_write_sample_2_bytes((uint16_t)data);
